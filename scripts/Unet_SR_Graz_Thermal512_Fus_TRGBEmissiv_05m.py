...
def unet1(input_size):
    ...
    model.compile(optimizer = Adam(learning_rate = 5e-4), loss = custom_loss, metrics = ['RootMeanSquaredError'])
    return model
...
# model1.summary()
# print('start***************************************')
# model0=load_model(cwd2 +'/Unet_SR_Thermal512org_3_Fus_05mGray_Emissiv_3_9it.h5', custom_objects={'custom_loss': custom_loss})
# # model0.summary() 
# for i in range(len(model0.layers)-5):
#     # print('name =',model1.layers[i+5].name)
#     if(model0.layers[i+5].name[0:4]=='conv'):
#         model1.layers[i+5].set_weights(model0.layers[i+5].get_weights())
        # print('-*******name lay=',model1.layers[i+5].name)
# # model1.layers[1].set_weights(vgg_16.layers[1].get_weights())
# model1.layers[2].set_weights(vgg_16.layers[2].get_weights())
# model1.layers[6].set_weights(vgg_16.layers[4].get_weights())
# model1.layers[7].set_weights(vgg_16.layers[5].get_weights())
# model1.layers[11].set_weights(vgg_16.layers[7].get_weights())
# model1.layers[12].set_weights(vgg_16.layers[8].get_weights())
# model1.layers[13].set_weights(vgg_16.layers[9].get_weights())
# model1.layers[17].set_weights(vgg_16.layers[11].get_weights())
# model1.layers[18].set_weights(vgg_16.layers[12].get_weights())
# model1.layers[19].set_weights(vgg_16.layers[13].get_weights())
# model1.layers[23].set_weights(vgg_16.layers[15].get_weights())
# model1.layers[24].set_weights(vgg_16.layers[16].get_weights())
# # model1.layers[25].set_weights(vgg_16.layers[17].get_weights())
...
# *...........................................................................................
# model1=load_model(cwd +'/Unet_SR_Thermal512_3_Fus_10mGray_4it.h5', custom_objects={'custom_loss': custom_loss})
...
dir_trEmissiv=cwd +'/Graz_dataset_512/Train/Input_Emissivity_05m/'
...
dir_valEmissiv=cwd +'/Graz_dataset_512/Val/Input_Emissivity_05m/'
...
# dir_tsEmissiv=cwd +'/Graz_dataset_512/Test/Input_emissivity_05m/'
...
# for i1 in range(1):
    ...
    # for i21 in range(50):
        for i22 in range(N2):
            ...
            img=np.load(dir_trEmissiv+train_files_name[i2][:]+'.npy')
            # print('img min max =',img.min(),' ',img.max())
            img=(img-0.7)*65
            train_imgs[i22,:,:,4] =img #[:,:,0]            
        ...
        print('Unet_SR_Graz_Thermal512_Fus_TRGBEmissiv_05m iteration number   ',iter1,' sub-iter = ',i21+1)
    ...
    np.save(cwd +'/Tr_Acc_Unet_SR_Graz_Thermal512_Fus_TRGBEmissiv_05m',tr_Acc)
    np.save(cwd +'/Tr_runtime_Unet_SR_Graz_Thermal512_Fus_TRGBEmissiv_05m',time_Tr)        
    ...
    for idi in range(Nv):
        ...
        img=img/div_RGB
        val_imgs[0,:,:,1:4] =img
        img=np.load(dir_valEmissiv+val_files_name[idi][:]+'.npy')
        img=(img-0.7)*65
        val_imgs[0,:,:,4] =img #[:,:,0]        
    ...
    np.save(cwd +'/Val_Acc_Unet_SR_Graz_Thermal512_Fus_TRGBEmissiv_05m',val_Acc)

    acc1=(val_Acc[iter1-1,1] + 2*(1-(np.mean(SSIMv))))/3
    if(acc1<MAE_min):
        MAE_min=acc1
        nb_min = 0
        model1.save(cwd +'/Unet_SR_Graz_Thermal512_Fus_TRGBEmissiv_05m_'+str(iter1)+'it.keras')
    else:
        nb_min=nb_min+1
    ...
    # np.save(cwd +'/Ts_Acc_Unet_SR_Graz_Thermal512_Fus_TRGBEmissiv_05m',ts_Acc)
