...
if(selected_model==1):
    model1=load_model(cwd +'/Unet_SR_Graz_Thermal512_Fus_TRGB_05m_12it.keras', custom_objects={'custom_loss': custom_loss}) # Graz mdl
else:
    if(selected_model==2):
        model1=load_model(cwd +'/Unet_SR_Ferrara_Thermal512_Fus_TRGB_05m_9it.keras', custom_objects={'custom_loss': custom_loss}) # Ferrara mdl
    else:
        if(selected_model==3):
            model1=load_model(cwd +'/Unet_SR_Graz_Ferrara_Thermal512_Fus_TRGB_05m_9it.keras', custom_objects={'custom_loss': custom_loss}) # Graz_Ferrara mdl
...
# im_Emmiss05m0=rasterio.open(cwd+'/Aligned_data_Ts_part/Ts_Part_Graz_day_surface_temperature_UTM33_mask_small_50cm_celcius_2.tif')
...
# Graz Model
if(selected_model==1):
    with rasterio.open(cwd+'/prediction/Pred_Graz_Ts_TRGB_Graz_mdl_LC08_190027_20210912_LST_C_small_aoi_2.tif', 'w', **profileT) as dst:
        dst.write(pred_label,1)
# Ferrara Model    
else:
    if(selected_model==2):
        with rasterio.open(cwd+'/prediction/Pred_Graz_Ts_TRGB_Ferrara_mdl_LC08_190027_20210912_LST_C_small_aoi_2.tif', 'w', **profileT) as dst:
            dst.write(pred_label,1)    
    else:
        # Graz_Ferrara Model
        if(selected_model==3):
            with rasterio.open(cwd+'/prediction/Pred_Graz_Ts_TRGB_Graz_Ferrara_mdl_LC08_190027_20210912_LST_C_small_aoi_2.tif', 'w', **profileT) as dst:
                dst.write(pred_label,1)
